...
func (s *Sound) Play(volume float64, pitch float64) error {
	...
	// Оборачиваем streamer чтобы закрыть файл при завершении
	ctrl := &closableStreamer{
		Streamer: streamer,
		file:     file,
	}

	// Применяем pitch shifting через resampler
	if pitch != 1.0 {
		ctrl = &closableStreamer{
...
// closableStreamer автоматически закрывает файл при завершении воспроизведения
...
// pitchShifter с кубической интерполяцией и анти-алиасингом
type pitchShifter struct {
	streamer beep.Streamer
	pitch    float64
	buf      [][2]float64
	bufPos   float64 // ДОЛЖНА БЫТЬ FLOAT64!
	bufEnd   int
	bufFull  bool
	history  [][2]float64 // сохраняем предыдущие семплы для интерполяции
}
...
func (ps *pitchShifter) Stream(samples [][2]float64) (n int, ok bool) {
	...
	if ps.buf == nil {
		...
		ps.history = make([][2]float64, 0, 4) // храним последние 4 семпла
		ps.bufEnd = 0
		ps.bufFull = false
	}
	...
	for outIdx < len(samples) {
		// Если позиция вышла за пределы буфера - загружаем новый
		if int(ps.bufPos) >= ps.bufEnd {
			// Сохраняем последние семплы перед загрузкой нового буфера
			if ps.bufEnd > 0 {
				start := ps.bufEnd - 3
				if start < 0 {
					start = 0
...
// Кубическая интерполяция с поддержкой истории буфера
...
// Получает семпл с поддержкой истории для интерполяции на границе буфера
func (ps *pitchShifter) getSampleWithHistory(idx int) [2]float64 {
	// Если индекс отрицательный, берем из истории
	if idx < 0 {
		histIdx := len(ps.history) + idx
		if histIdx >= 0 && histIdx < len(ps.history) {
			return ps.history[histIdx]
		}
		// Если истории нет, повторяем первый семпл
		if ps.bufEnd > 0 {
			return ps.buf[0]
		}
	...
	// Если индекс в текущем буфере
	if idx < ps.bufEnd {
		return ps.buf[idx]
	}

	// Если за пределами буфера, возвращаем нуль
	return [2]float64{0, 0}
}
